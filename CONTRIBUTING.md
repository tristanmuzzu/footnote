...
Thanks for wanting to make footnote better. It's intentionally small — you should
be able to read the whole thing in a few minutes.

## Project layout

| Path | What it owns |
| --- | --- |
| `hooks/footnote-activate.js` | **SessionStart hook.** The rules Claude receives each session + injects the current log. This is the always-on engine. |
| `hooks/footnote-mode-tracker.js` | **UserPromptSubmit hook.** Detects `footnote off` / `footnote on` and flips the mute flag. |
...
| `.claude-plugin/plugin.json` | Plugin manifest (wires the hooks). |
| `.claude-plugin/marketplace.json` | Lets people install via `/plugin marketplace add`. |
| `README.md` | The product front door. Optimize it for someone non-technical deciding whether to install. |

## The one rule that matters

**The behavior is defined in two places and they must agree:** the rules string
...
- **Non-destructive:** only ever read/write under `~/.claude/footnote/`.
...
The hooks are plain Node scripts. You can run them by hand:

```bash
# SessionStart: should print the rules + your current log to stdout
...
Spaced-repetition recaps, per-domain hint tuning (data science, devops, etc.),
weekly summaries, export formats — open an issue and let's talk.
